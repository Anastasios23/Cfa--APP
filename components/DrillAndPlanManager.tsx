    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. Cones, Balls, Vests"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
        />
      </div>
      <div>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {selectedDrills.map((pd) => {
          ...
          return (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => handleRemoveDrill(pd.drillId)}
        ...
        {selectedDrills.length === 0 && (
          <p className="text-sm text-gray-500">No drills added yet.</p>
        )}
      </div>
        ...
        {drills.map((drill) => (
          ...
          >
            ...
            <Button
              ...
              disabled={!!selectedDrills.find((pd) => pd.drillId === drill.id)}
            >
              Add
            </Button>
          </div>
...
export const DrillAndPlanManager: React.FC<DrillAndPlanManagerProps> = ({
  drills,
  plans,
  addDrill,
  addPlan,
  ...
  const [activeTab, setActiveTab] = useState<"library" | "drills" | "plans">(
    "library"
  );

  const handleSavePlan = (
    planData: Omit<TrainingPlan, "id"> | TrainingPlan
  ) => {
    if ("id" in planData) {
      updatePlan(planData);
    } else {
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <Modal
        ...
        title={`Plan Details: ${viewingPlan?.name}`}
      >
        {viewingPlan && (
            ...
            <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
              {viewingPlan.drills.map((planDrill, index) => {
                ...
                return (
                  ...
                  >
                    <h4 className="font-bold text-md text-brand-blue">
                      {index + 1}. {drill.name} ({planDrill.duration} min)
                    </h4>
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <span className="text-xs bg-brand-green text-white px-2 py-1 rounded-full inline-block">
                        {drill.category}
                      </span>
                      {drill.videoUrl && (
                        <Button
                          variant="secondary"
                          onClick={() => setViewingVideoUrl(drill.videoUrl)}
                        >
                          <PlayIcon className="w-5 h-5 mr-2 inline" /> Watch
                          Video
                        </Button>
                      )}
                    </div>
      ...
      {/* Tab Navigation */}
      <div className="flex border-b border-gray-200 mb-6 max-w-6xl mx-auto">
        <button
      ...
      {activeTab === "library" && (
        <DrillLibrary
          drills={drills}
          onViewDrill={(drill) => {
            setExpandedDrillId(drill.id);
          }}
          onPlayVideo={(videoUrl) => setViewingVideoUrl(videoUrl)}
        />
      )}
          ...
          <Card>
            <div className="flex justify-between items-center mb-6">
              ...
              <Button onClick={() => setIsDrillModalOpen(true)}>
                <PlusIcon className="w-5 h-5 mr-2 inline" /> Add New Drill
              </Button>
            </div>
            {drills.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <p>No drills yet. Create your first drill!</p>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        onClick={() =>
                          setExpandedDrillId(isExpanded ? null : drill.id)
                        }
                      >
                        ...
                        <span className="text-xs bg-brand-green text-white px-2 py-1 rounded-full mt-2 inline-block">
                          {drill.category}
                        </span>
                      </div>
                      {isExpanded && (
                          ...
                          {drill.videoUrl && (
                            <div className="pt-2">
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  setViewingVideoUrl(drill.videoUrl)
                                }
                              >
          ...
          <Card>
            <div className="flex justify-between items-center mb-6">
              ...
              <Button onClick={() => setIsPlanModalOpen(true)}>
                <PlusIcon className="w-5 h-5 mr-2 inline" /> Create New Plan
              </Button>
            </div>
            {plans.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <p>No training plans yet. Create your first plan!</p>
              </div>
            ) : (
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {plans.map((plan) => (
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center space-x-2">
                        <button
                          ...
                          className="text-brand-blue hover:text-blue-800 transition-colors p-1 rounded-full hover:bg-blue-100"
                        >
