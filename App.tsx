...
const App: React.FC = () => {
  ...
  const addPlan = (plan: Omit<TrainingPlan, "id">): TrainingPlan => {
    ...
    setData((prev) => ({
      ...prev,
      trainingPlans: [...prev.trainingPlans, newPlan],
    }));
    return newPlan;
  };
  ...
  const updatePlan = (updatedPlan: TrainingPlan) => {
    setData((prev) => ({
      ...prev,
      trainingPlans: prev.trainingPlans.map((p) =>
...
const DashboardCard: React.FC<{
  ...
  <button
    onClick={onClick}
    className="text-left bg-white p-6 rounded-xl shadow-md hover:shadow-lg hover:scale-105 transition-transform duration-200 ease-in-out w-full"
  >
